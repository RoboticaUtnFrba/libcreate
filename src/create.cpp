...
void Create::onData()
{
  ...
  const float kr = 1.0;  // TODO(eborghi10): Perform experiments to find these nondeterministic parameters
  const float kl = 1.0;
...
//  serial->reset(); // better
// TODO(eborghi10) : Should we request reading packets again?
// }
...
bool Create::driveWheels(const float& leftVel, const float& rightVel)
{
  ...
  else
  {
    ...
    {
      const float tmp_vel = (std::abs(boundedLeftVel) + std::abs(boundedRightVel)) / 2.0;
      vel = tmp_vel * ((boundedLeftVel + boundedRightVel) > 0 ? 1.0 : -1.0);
    }
    ...
    // TODO(eborghi10): alternate between these 2 meter radius and straight to
